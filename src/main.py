...
CONFIG_PFILE_PATH = 'json_files/config.json'
...
if __name__ == '__main__':
    init_parsers()
    providers = load_json('json_files/providers.json')
    
    if providers.get('config_template'):
        print ('选择: \033[33m' + providers['config_template'] + '\033[0m')
        response = requests.get(providers['config_template'])
        response.raise_for_status()
        config = response.json()
    else:
        print ('选择: \033[33m' + 'json_files/config_template.json' + '\033[0m')
        config = load_json('json_files/config_template.json')
    ...
    save_config(CONFIG_PFILE_PATH, final_config)